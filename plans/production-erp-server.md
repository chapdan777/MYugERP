...
Изменяет базовую цену номенклатуры на основе выбранных свойств. Поддерживает два способа определения условий применения:

**Способ 1: Простые условия (устаревший)**
- `propertyID` + `propertyValue` - базовое сопоставление

**Способ 2: Сложные условия (рекомендуемый)**
- `conditionExpression` - SQL-подобный синтаксис для гибких условий

| Поле | Тип | Описание |
|------|-----|----------|
| id | UUID | Уникальный идентификатор |
| productID | UUID | Ссылка на номенклатуру (опционально - может применяться глобально) |
| propertyID | UUID | Свойство, вызывающее модификатор (для простых условий) |
| propertyValue | string | Значение свойства, вызывающее модификатор (для простых условий) |
| conditionExpression | string | Сложное условие фильтрации (для гибких условий) |
...
**Система условий фильтрации:**

Модификаторы могут применяться при выполнении различных условий:

**Простые условия:**
- `propertyId = 123 AND propertyValue = 'премиум'`

**Сложные условия (рекомендуемые):**
- Сравнения: `propertyValue > 1000`, `propertyValue < 5000`
- Поиск по шаблону: `propertyValue LIKE 'цвет:%'`
- Списки: `propertyValue IN ('красный', 'синий', 'зеленый')`
- Диапазоны: `propertyValue BETWEEN 1000 AND 5000`
- Логические операции: `AND`, `OR`, `NOT`
- Группировка скобками: `(условие1) AND (условие2)`

**Примеры бизнес-кейсов:**
- Надбавка 10% для товаров дороже 3000 руб: `propertyValue > 3000`
- Скидка 5% для определенных цветов: `propertyValue IN ('белый', 'бежевый')`
- Специальный модификатор для товаров в диапазоне 1500-4000 руб: `propertyValue BETWEEN 1500 AND 4000`
- Комбинированное условие: `(propertyId = 1 AND propertyValue = 'фасад') AND (propertyValue > 2000)`
